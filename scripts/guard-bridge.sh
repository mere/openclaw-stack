#!/usr/bin/env bash
set -euo pipefail
ACTION=${1:-help}
PENDING=/var/lib/openclaw/guard-state/bridge/pending.json
POLICY=/var/lib/openclaw/guard-state/bridge/policy.json

case "$ACTION" in
  run-once)
    exec /root/openclaw-stack/scripts/guard-bridge-runner.py
    ;;
  pending)
    cat "$PENDING" 2>/dev/null || echo '{}'
    ;;
  policy)
    cat "$POLICY" 2>/dev/null || echo '{}'
    ;;
  approve|reject)
    REQ=${2:-}
    MODE=${3:-once} # once|always
    [ -n "$REQ" ] || { echo "usage: $0 $ACTION <requestId> [once|always]"; exit 1; }
    python3 - "$ACTION" "$REQ" "$MODE" <<'PY'
import json, pathlib, subprocess, sys
action, req_id, mode = sys.argv[1], sys.argv[2], sys.argv[3]
pending_p=pathlib.Path('/var/lib/openclaw/guard-state/bridge/pending.json')
policy_p=pathlib.Path('/var/lib/openclaw/guard-state/bridge/policy.json')
outbox=pathlib.Path('/var/lib/openclaw/bridge/outbox'); outbox.mkdir(parents=True, exist_ok=True)

def now():
    import datetime
    return datetime.datetime.utcnow().replace(microsecond=0).isoformat()+"Z"

pending=json.loads(pending_p.read_text() if pending_p.exists() else '{}')
item=pending.get(req_id)
if not item:
    print('request_not_found')
    sys.exit(1)
req=item['request']
act=req['action']
policy=json.loads(policy_p.read_text() if policy_p.exists() else '{}')

if mode=='always':
    policy[act]='approved' if action=='approve' else 'rejected'
    policy_p.parent.mkdir(parents=True, exist_ok=True)
    policy_p.write_text(json.dumps(policy, indent=2)+'\n')

if action=='reject':
    out=(outbox/f'{req_id}.json')
    out.write_text(json.dumps({'requestId':req_id,'status':'rejected','error':'manual_reject','completedAt':now()}, indent=2)+'\n')
else:
    cmd=['/root/openclaw-stack/scripts/guard-email.sh', act, json.dumps(req.get('args',{}))]
    pr=subprocess.run(cmd, capture_output=True, text=True)
    raw=(pr.stdout or '').strip() or (pr.stderr or '').strip()
    try:
        res=json.loads(raw) if raw else {'ok': pr.returncode==0}
    except Exception:
        res={'ok': pr.returncode==0, 'raw': raw}
    status='ok' if pr.returncode==0 else 'error'
    out=(outbox/f'{req_id}.json')
    out.write_text(json.dumps({'requestId':req_id,'status':status,'action':act,'result':res,'completedAt':now()}, indent=2)+'\n')

pending.pop(req_id, None)
pending_p.write_text(json.dumps(pending, indent=2)+'\n')
print('done')
PY
    ;;
  *)
    cat <<EOF
Usage:
  $0 run-once
  $0 pending
  $0 policy
  $0 approve <requestId> [once|always]
  $0 reject <requestId> [once|always]
EOF
    ;;
esac
